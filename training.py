...
def create_mask_tensor(input_tensor, n):
    col_indices = torch.arange(n, device=input_tensor.device).unsqueeze(0)
    expanded_input = input_tensor.expand(-1, n)
    output_tensor = (col_indices < expanded_input).bool()
...
class Predict2Video2WorldModelLightning(L.LightningModule):
    def __init__(
        ...
        self.num_action_tokens = 8
        gaussian_dist = NormalDist(mu=0.0, sigma=1.0)
        cdf_vals = np.clip(np.linspace(0, 1.0, self.num_action_tokens + 1), 1e-8, 1 - 1e-8)
        samples_interval_gaussian = [gaussian_dist.inv_cdf(cdf_val) for cdf_val in cdf_vals]
        self.sigma_bins = nn.Parameter(torch.tensor(samples_interval_gaussian).exp(), requires_grad=False)

        total_params = sum(p.numel() for p in self.parameters())
        frozen_params = sum(p.numel() for p in self.parameters() if not p.requires_grad)
        trainable_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        # Print the number in billions, or in the format of 1,000,000,000
        log.info(f"Total parameters: {total_params / 1e9:.2f}B, Frozen parameters: {frozen_params:,}, Trainable parameters: {trainable_params:,}")
    ...
    def draw_training_sigma_and_epsilon(self, x0_size: torch.Size, condition: Any) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if is_video_batch and self.high_sigma_ratio > 0:
            ...
            mask = torch.rand(sigma_B_1.shape, device=sigma_B_1.device) < self.high_sigma_ratio
            log_new_sigma = torch.rand(sigma_B_1.shape, device=sigma_B_1.device).type_as(sigma_B_1) * (LOG_100000 - LOG_200) + LOG_200
    ...
    def training_step(self, batch, batch_idx):
        self.pipe._normalize_video_databatch_inplace(batch)
        self.pipe._augment_image_dim_inplace(batch)
        ...
        condition = condition.set_video_condition(
            gt_frames=x0_B_C_T_H_W.to(**self.pipe.tensor_kwargs),
            random_min_num_conditional_frames=self.pipe.config.min_num_conditional_frames,
            random_max_num_conditional_frames=self.pipe.config.max_num_conditional_frames,
        ...
        token_indices = torch.searchsorted(self.sigma_bins, sigma_B_T, side="right") - 1
        padding_mask = create_mask_tensor(token_indices + 1, self.num_action_tokens)
        batch["attention_bias"] = padding_mask

        output_batch, kendall_loss, _, _ = self.compute_loss_with_epsilon_and_sigma(x0_B_C_T_H_W, condition, epsilon_B_C_T_H_W, sigma_B_T)
...
class CosmosDatasetWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            log.warning(f"Task '{task}' not found in T5 embeddings. Using zero embeddings.")
...
dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, num_workers=11, shuffle=True, persistent_workers=True)
