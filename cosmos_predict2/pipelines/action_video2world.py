...
class ActionConditionedVideo2WorldPipeline(Video2WorldPipeline):
    ...
    def from_config(
        ...
        # Create a pipe
        pipe = ActionConditionedVideo2WorldPipeline(
    ...
    def _get_data_batch_input(
        ...
        data_batch = {
            ...
            "t5_text_embeddings": torch.zeros(
                self.batch_size, 512, 1024, dtype=torch.bfloat16
            ).cuda(),
            "fps": torch.randint(
...
class SimpleActionConditionedVideo2WorldPipeline(SimpleVideo2WorldPipeline):
    def __init__(self, device: str = "cuda", torch_dtype: torch.dtype = torch.bfloat16):
        super().__init__(device=device, torch_dtype=torch_dtype)
    ...
    def from_config(
        ...
        # Create a pipe
        pipe = SimpleActionConditionedVideo2WorldPipeline(
            device=device, torch_dtype=torch_dtype
        )
        pipe.config = config
        pipe.precision = {
