...
def resize_input(video: torch.Tensor, resolution: list[int]) -> torch.Tensor:
    ...
    video_cropped = torchvision.transforms.functional.center_crop(
        video_resized, resolution
    )
...
def read_and_process_image(
    ...
    if resize:
        ...
        img_resized = torchvision.transforms.functional.resize(
            img.unsqueeze(0), resizing_shape
        ).squeeze(0)
    ...
    vid_input = torch.zeros(
        (num_video_frames, C, H, W), dtype=torch.uint8, device=img.device
    )
...
def read_and_process_video(
    ...
    try:
        video_frames, video_metadata = easy_io.load(
            video_path
        )  # Returns (T, H, W, C) numpy array
        log.info(
            f"Loaded video with shape {video_frames.shape}, metadata: {video_metadata}"
        )
    ...
    video_tensor = (
        torch.from_numpy(video_frames).float() / 255.0
    ...
    log.info(
        f"Will extract the last {frames_to_extract} frames from input video and pad to {num_video_frames}"
    )
    ...
    extracted_frames = extracted_frames.permute(
        1, 0, 2, 3
    )  # (frames_to_extract, C, H, W)
    ...
    if resize:
        ...
        resizing_shape = (
            int(math.ceil(scaling_ratio * H)),
            int(math.ceil(scaling_ratio * W)),
        )

        # Resize and crop the extracted frames
        extracted_frames = torchvision.transforms.functional.resize(
            extracted_frames, resizing_shape
        )
        extracted_frames = torchvision.transforms.functional.center_crop(
            extracted_frames, resolution
        )

        # Resize and crop the last frame separately (for padding)
        last_frame = last_frame.unsqueeze(0)  # Add batch dim for resize
        last_frame = torchvision.transforms.functional.resize(
            last_frame, resizing_shape
        )
        last_frame = torchvision.transforms.functional.center_crop(
            last_frame, resolution
        )
    ...
    full_video = torch.zeros(
        (num_video_frames, C, H, W), dtype=torch.uint8, device=extracted_frames.device
    )
...
class SimpleVideo2WorldPipeline(BasePipeline):
    def __init__(self, device: str = "cuda", torch_dtype: torch.dtype = torch.bfloat16):
        super().__init__(device=device, torch_dtype=torch_dtype)
        self.text_encoder: CosmosT5TextEncoder = None
        self.dit: torch.nn.Module = None
    ...
    def from_config(
        ...
        # Create a pipe
        pipe = SimpleVideo2WorldPipeline(device=device, torch_dtype=torch_dtype)
        pipe.config = config
        pipe.precision = {
            "float32": torch.float32,
            "float16": torch.float16,
        ...
        pipe.scaling = RectifiedFlowScaling(
            pipe.sigma_data, config.rectified_flow_t_scaling_factor
        )
        ...
        for p in pipe.tokenizer.parameters():
        ...
        if text_encoder_path:
            ...
            pipe.text_encoder = CosmosT5TextEncoder(
                device=device, cache_dir=text_encoder_path
            )
        ...
        pipe.conditioner = instantiate(config.conditioner)
        assert (
            sum(p.numel() for p in pipe.conditioner.parameters() if p.requires_grad)
            == 0
        ), "conditioner should not have learnable parameters"
        ...
        if config.ema.enabled:
            ...
            s = config.ema.rate
            pipe.ema_exp_coefficient = np.roots(
                [1, 7, 16 - s**-2, 12 - s**-2]
            ).real.max()
            # copying is only necessary when starting the training at iteration 0.
            # Actual state_dict should be loaded after the pipe is created.
    ...
    def setup_data_key(self) -> None:
        ...
        )  # by default it is video key for Video diffusion model
        self.input_image_key = self.config.input_image_key
    ...
    def apply_fsdp(self, dp_mesh: DeviceMesh) -> None:
        ...
        if self.dit_ema:
            self.dit_ema.fully_shard(mesh=dp_mesh)
            self.dit_ema = fully_shard(
                self.dit_ema, mesh=dp_mesh, reshard_after_forward=True
            )
            broadcast_dtensor_model_states(self.dit_ema, dp_mesh)
            self.dit_ema_worker = DTensorFastEmaModelUpdater()
            # No need to copy weights to EMA when applying FSDP, it is already copied before applying FSDP.
    ...
    def _get_data_batch_input(
        ...
        t5_text_embeddings: torch.Tensor | None = None,
    ):
        """
        Prepares the input data batch for the diffusion model.
        ...
        data_batch = {
            ...
            "fps": torch.ones((self.batch_size,), dtype=torch.long) * 5,
            # "fps": torch.randint(
            #     16, 32, (self.batch_size,)
            # ),  # Random FPS (might be used by model)
        ...
        # Handle negative prompts for classifier-free guidance
        if negative_prompt:
            data_batch["neg_t5_text_embeddings"] = self.encode_prompt(
                negative_prompt
    ...
    def encode_prompt(
        ...
        return self.text_encoder.encode_prompts(
            prompts, max_length=max_length, return_mask=return_mask
        )  # type: ignore
    ...
    def denoise(
        ...
        if condition.is_video:
            condition_state_in_B_C_T_H_W = (
                condition.gt_frames.type_as(net_state_in_B_C_T_H_W)
            ...
            condition_video_mask = (
                condition.condition_video_input_mask_B_C_T_H_W.repeat(
                    1, C, 1, 1, 1
                ).type_as(net_state_in_B_C_T_H_W)
            )

            if self.config.conditioning_strategy == str(
                ConditioningStrategy.FRAME_REPLACE
            ):
                # In case of frame replacement strategy, replace the first few frames of the video with the conditional frames
                # Update the c_noise as the conditional frames are clean and have very low noise
                ...
                # Adjust c_noise for the conditional frames
                sigma_cond_B_1_T_1_1 = (
                    torch.ones_like(sigma_B_1_T_1_1) * self.config.sigma_conditional
                )
                _, _, _, c_noise_cond_B_1_T_1_1 = self.scaling(
                    sigma=sigma_cond_B_1_T_1_1
                )
                condition_video_mask_B_1_T_1_1 = condition_video_mask.mean(
                    dim=[1, 3, 4], keepdim=True
                )
                c_noise_B_1_T_1_1 = (
                    c_noise_cond_B_1_T_1_1 * condition_video_mask_B_1_T_1_1
                    + c_noise_B_1_T_1_1 * (1 - condition_video_mask_B_1_T_1_1)
                )
            elif self.config.conditioning_strategy == str(
                ConditioningStrategy.CHANNEL_CONCAT
            ):
                # In case of channel concatenation strategy, concatenate the conditional frames in the channel dimension
                condition_state_in_masked_B_C_T_H_W = (
                    condition_state_in_B_C_T_H_W * condition_video_mask
                )
                net_state_in_B_C_T_H_W = torch.cat(
                    [net_state_in_B_C_T_H_W, condition_state_in_masked_B_C_T_H_W], dim=1
                )
    ...
    def get_x0_fn_from_batch(
        ...
        def x0_fn(noise_x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
            cond_x0 = self.denoise(
                noise_x, sigma, condition, use_cuda_graphs=use_cuda_graphs
            ).x0
            uncond_x0 = self.denoise(
                noise_x, sigma, uncondition, use_cuda_graphs=use_cuda_graphs
            ).x0
            raw_x0 = cond_x0 + guidance * (cond_x0 - uncond_x0)
            if "guided_image" in data_batch:
                # replacement trick that enables inpainting with base model
    ...
    def __call__(
        ...
        # Parameter check
        # width, height = VIDEO_RES_SIZE_INFO[self.config.resolution][aspect_ratio]
        height, width = self.check_resize_height_width(*image_sz)
        assert num_conditional_frames in [1, 5], "num_conditional_frames must be 1 or 5"
        num_latent_conditional_frames = self.tokenizer.get_latent_num_frames(
        ...
        if ext in _VIDEO_EXTENSIONS:
            ...
            if num_latent_conditional_frames == 1:
                ...
                vid_input = read_and_process_image(
                    input_path, [height, width], num_video_frames, resize=True
                )
            else:
                raise ValueError(
                    f"Cannot use multi-frame conditioning (num_conditional_frames={num_conditional_frames}) with image input. Please provide a video file."
        ...
        # vid_input: (B, C, T, H, W) uint8, zero padding for future frames
        ...
        # Split the input data and condition for model parallelism, if context parallelism is enabled.
        if self.dit.is_context_parallel_enabled:
            x_sigma_max = split_inputs_cp(
                x=x_sigma_max, seq_dim=2, cp_group=self.get_context_parallel_group()
            )
        ...
        for i, _ in enumerate(
            tqdm(scheduler.timesteps, desc="Generating world", leave=False)
        ):
            # Current noise level (sigma_t).
            sigma_t = scheduler.sigmas[i].to(sample.device, dtype=torch.float32)
        ...
        if self.dit.is_context_parallel_enabled:
            samples = cat_outputs_cp(
                samples, seq_dim=2, cp_group=self.get_context_parallel_group()
            )
        ...
        #     # Convert processed frames back to tensor format
        #     processed_video = (
        #         torch.from_numpy(processed_frames).float().permute(3, 0, 1, 2) / 255.0
        #     )
        #     processed_video = processed_video * 2 - 1  # back to [-1, 1]
        #     processed_video = processed_video.unsqueeze(0)
